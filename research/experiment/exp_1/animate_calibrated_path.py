"""
Animated Trajectory Visualizer for a New Calibrated Driving Path
Applies Optimal Calibration Factors (k_accel = 1.1379, k_gyro = 0.9794)
"""
...
def animate_new_calibrated_route():
    ...
    # Generate a fresh, challenging multi-turn route (seed=2026, mixed driving, 150 seconds)
    duration = 150
    ...
    print(f"[Animate] Route: {route_name}, Total Distance: {total_gt_distance:.2f} m")
    print(f"[Animate] Final Drift: {final_drift:.2f} m ({(final_drift/total_gt_distance)*100:.2f}%)")
    print(f"[Animate] Mean ATE:    {mean_ate:.2f} m ({(mean_ate/total_gt_distance)*100:.2f}%)")

    # Render High-Quality Animated GIF
    print("[Animate] Rendering 150s animation frames...")
    ...
    ax_map.set_title("Calibrated Neural Odometry: New Route (1.8 km Drive)", color='#00f0ff', fontsize=12, fontweight='bold')
    ...
    ax_map.plot(pred_x, pred_y, color='#00f0ff', linestyle=':', alpha=0.35, label='Calibrated AI Path (Full)')

    line_gt, = ax_map.plot([], [], color='#3fb950', linewidth=2.8, label='Ground Truth Path')
    line_pred, = ax_map.plot([], [], color='#00f0ff', linewidth=2.8, label='Calibrated AI Path')
    ...
    ax_speed.set_title("Calibrated Kinematic Speed (km/h)", color='#f0883e', fontsize=11, fontweight='bold')
    ...
    gif_out = os.path.join(REPORT_DIR, "trajectory_calibrated_new.gif")
    anim.save(gif_out, writer='pillow', fps=12)
    plt.close(fig)
    print(f"[Animate] Saved animation to {gif_out} ({os.path.getsize(gif_out):,} bytes)!")


if __name__ == "__main__":
    animate_new_calibrated_route()
